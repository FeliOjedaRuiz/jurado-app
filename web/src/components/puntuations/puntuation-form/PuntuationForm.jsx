...
function PuntuationForm({ group, onSave }) {
  ...
  const onPuntuationSubmit = async (puntuation) => {
    try {
      ...
      if (error.response.status === 403) {
        setServerError("No está hábilitada la votación para este evento, consulta al administrador.");
      } else {
  ...
  return (
    ...
    >
      ...
      {serverError && (
        <div className="text-red-800 text-md font-medium text-center">{serverError}</div>
      )}
      <button
        type="submit"
